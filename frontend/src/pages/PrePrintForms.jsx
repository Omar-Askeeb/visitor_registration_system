            ...
            <form onSubmit={handleSaveAndCalculate} className="space-y-6">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/40"
                >
                  {events.map(ev => (
                    <option key={ev.id} value={ev.id}>{ev.name} (Prefix: {ev.form_id_prefix})</option>
                  ))}
                </select>
              </div>
