<?php
// fix_missing_scans.php
// Run this from the browser to fix missing scans for visitors who printed their badges.
require_once 'config.php';

echo "<h2>Starting scan synchronization...</h2>";

try {
    // Detect table names based on context (local Laravel DB uses v2_ prefix, dashboard uses without prefix depending on setup)
    $tables = $pdo->query("SHOW TABLES")->fetchAll(PDO::FETCH_COLUMN);
    $visitorTable = in_array('v2_visitors', $tables) ? 'v2_visitors' : 'visitors';
    $scanTable = in_array('v2_scans', $tables) ? 'v2_scans' : 'scans';

    echo "Using Visitor Table: <strong>$visitorTable</strong><br>";
    echo "Using Scan Table: <strong>$scanTable</strong><br><br>";

    // Ensure we know available columns
    ...
    while ($visitor = $visitors->fetch(PDO::FETCH_ASSOC)) {
        // Check if barcode already exists in scans
        ...
        if ($exists == 0) {
            ...
            // Prefer print_date if available
            ...
            try {
                ...
                echo "<span style='color:red;'>Error inserting for barcode " . htmlspecialchars($visitor['badgeID']) . ": " . $e->getMessage() . "</span><br>";
                $errors++;
            }
        }
    }
    ...
    echo "<strong>Added $added missing scan records.</strong><br>";
    if ($errors > 0) {
        echo "<strong style='color:red;'>Errors encountered: $errors</strong><br>";
    }
    echo "<br><a href='index.php'>Return to Dashboard</a>";
    
} catch (Exception $e) {
    echo "<h3 style='color:red;'>Fatal Error: " . htmlspecialchars($e->getMessage()) . "</h3>";
}
?>
